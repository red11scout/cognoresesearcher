...
// Merges overlapping patterns from original 8 + new patterns from Agentic Design Pattern docs
...
export const LEGACY_PATTERN_MAP: Record<string, AgenticPattern> = {
  ...
  "Human-in-the-Loop": "Constitutional Guardrail", // HITL is now a cross-cutting flag, not a pattern
};

// Resolve a pattern name (handles legacy names)
export function resolvePatternName(name: string): AgenticPattern {
  if (AGENTIC_PATTERNS.includes(name as AgenticPattern)) return name as AgenticPattern;
  return LEGACY_PATTERN_MAP[name] || "Prompt Chaining";
}
...
export const AGENTIC_PATTERN_META: Record<AgenticPattern, {
  ...
  "Planning": {
    ...
    type: "single-agent",
    complexity: "medium",
    useCaseExamples: ["Strategic analysis", "Multi-step research", "Project planning"]
  },
  "ReAct Loop": {
    description: "Alternates between reasoning (Thought) and action (tool calls) in an iterative loop",
    icon: "🔄",
    type: "single-agent",
    complexity: "medium",
    useCaseExamples: ["Technical diagnostics", "Root cause analysis", "Exploratory analysis"]
  },
  "Prompt Chaining": {
    description: "Sequential pipeline where the output of one step becomes input to the next",
    icon: "🔗",
    type: "single-agent",
    complexity: "low",
    useCaseExamples: ["Data pipelines", "Report generation", "ETL workflows"]
  },
  "Semantic Router": {
    description: "Classifies input and routes to the most appropriate specialist handler",
    icon: "🔀",
    type: "single-agent",
    complexity: "low",
  ...
  "Constitutional Guardrail": {
    description: "Compliance-sensitive outputs with built-in safety and regulatory constraints",
    icon: "🛡️",
    type: "single-agent",
    complexity: "medium",
  ...
  "Orchestrator-Workers": {
    description: "Central manager agent coordinates and delegates to multiple specialist agents",
    icon: "🎭",
    type: "multi-agent",
    complexity: "high",
    useCaseExamples: ["Enterprise workflows", "Complex document processing", "Multi-department analysis"]
  },
  "Agent Handoff": {
    description: "Decentralized agents dynamically pass control based on task requirements",
    icon: "🤝",
    type: "multi-agent",
    complexity: "medium",
    useCaseExamples: ["Customer service escalation", "Multi-expertise tasks", "Specialized routing"]
  },
  "Parallelization": {
    description: "Multiple agents run simultaneously on sub-tasks, results merged at completion",
    icon: "⚡",
    type: "multi-agent",
    complexity: "high",
    useCaseExamples: ["Data extraction at scale", "Multi-perspective brainstorming", "Parallel analysis"]
  },
  "Generator-Critic": {
    description: "Two-agent loop: one generates content, another reviews and critiques until approved",
    icon: "✍️",
    type: "multi-agent",
    complexity: "medium",
    useCaseExamples: ["Content creation & review", "Code generation & validation", "Compliance checking"]
  },
  "Group Chat": {
    description: "Team of agents in collaborative dialogue, moderated by a lightweight facilitator",
    icon: "💬",
    type: "multi-agent",
    complexity: "high",
    useCaseExamples: ["Strategy brainstorming", "Policy development", "Multi-stakeholder planning"]
  },
};
...
// ============================================
// WHAT-IF SCENARIOS: Saved calculation scenarios for comparison
// ============================================
export const whatIfScenarios = pgTable("what_if_scenarios", {
  ...
  name: text("name").notNull(),
  description: text("description"),
  assumptionsJson: jsonb("assumptions_json").notNull(), // changed assumptions
  resultsJson: jsonb("results_json").notNull(), // recalculated results
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
// ============================================
// CUSTOM DATA: User-added themes, friction points, use cases
// ============================================
export const customData = pgTable("custom_data", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  reportId: varchar("report_id").notNull(),
  dataType: varchar("data_type", { length: 30 }).notNull(), // 'theme' | 'friction_point' | 'use_case'
  dataJson: jsonb("data_json").notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});

export const insertCustomDataSchema = createInsertSchema(customData).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
export type InsertCustomData = z.infer<typeof insertCustomDataSchema>;
export type CustomData = typeof customData.$inferSelect;
...
export interface CustomFrictionPoint {
  isCustom: true;
  "Function": string;
  "Sub-Function": string;
  "Role": string;
...
export interface CustomUseCase {
  ...
  "Primary Agentic Pattern"?: string;
  "Alternative Agentic Pattern"?: string;
}

// ============================================
// AGENTIC PATTERN TYPES: Enhanced pattern support
// ============================================
...
export interface PatternComparison {
  ...
  primaryPatternType: AgenticPatternType;
  alternativePattern: string;
  alternativePatternType: AgenticPatternType;
  comparison: {
    complexity: { primary: PatternComplexity; alternative: PatternComplexity };
    estimatedCostMultiplier: { primary: number; alternative: number };
    timeToValue: { primary: number; alternative: number }; // months
    reliability: { primary: string; alternative: string };
    tokenMultiplier: { primary: number; alternative: number };
  };
  rationale: string;
  epochFlags?: {
    ethical: boolean;
