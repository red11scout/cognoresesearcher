...
import { AGENTIC_PATTERNS, resolvePatternName, AGENTIC_PATTERN_META, type AgenticPattern } from '../shared/schema';
import { getPatternById, AGENTIC_PATTERN_CATALOG, type AgenticPatternDefinition } from '../shared/agenticPatterns';
...
export function postProcessAnalysis(analysisResult: any): any {
  ...
  if (step4?.data && Array.isArray(step4.data)) {
    ...
    for (const record of step4.data) {
      ...
      const primaryDef = getPatternById(record["Primary Pattern"].toLowerCase().replace(/\s+/g, '-'));
      if (primaryDef) {
  ...
  if (step6?.data && Array.isArray(step6.data)) {
    ...
    for (const record of step6.data as Step6Record[]) {
      ...
      // Apply pattern-aware token multiplier from Step 4 metadata
      let patternTokenMultiplier = 1.0;
      if (step4?.data && Array.isArray(step4.data)) {
        const step4Match = (step4.data as any[]).find(r => r.ID === record.ID);
        if (step4Match?.["_tokenMultiplier"]) {
          patternTokenMultiplier = step4Match["_tokenMultiplier"];
        }
      }
      const adjustedAnnualCost = tokenResult.annualCost * patternTokenMultiplier;
      const adjustedMonthlyTokens = Math.round(tokenResult.monthlyTokens * patternTokenMultiplier);
      totalMonthlyTokens += adjustedMonthlyTokens;
      ...
      // Build record with new column order (using pattern-adjusted token costs)
      const orderedRecord: Record<string, any> = {
        ...
        "Governance": gov,
        "Time To Value": ttv,
        "Monthly Tokens": adjustedMonthlyTokens,
        "Runs/Month": record["Runs/Month"],
        "Input Tokens/Run": record["Input Tokens/Run"],
        "Output Tokens/Run": record["Output Tokens/Run"],
        "Annual Token Cost": formatMoney(adjustedAnnualCost),
        "Pattern Token Multiplier": patternTokenMultiplier,
      };
