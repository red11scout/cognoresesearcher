  ...
  if (matchingTemplate) {
    ...
    const targetWorkflow: TargetWorkflowStep[] = matchingTemplate.targetStateWorkflow.map(step => ({
      ...
      agentType: step.isHumanInTheLoop
        ? "Constitutional Guardrail" as AgenticPattern
...
function validateAndCorrectWorkflow(workflowData: any, useCase: UseCase): any {
  ...
  if (!hasHITL && targetSteps.length > 0) {
    ...
    targetSteps[midIndex].agentType = "Constitutional Guardrail";
  ...
  targetSteps.forEach((step: TargetWorkflowStep, index: number) => {
    ...
    step.agentType = step.agentType || (step.isAIEnabled ? selectedPattern : (step.isHumanInTheLoop ? "Constitutional Guardrail" : selectedPattern));
...
const PATTERN_PRIMITIVE_SCORES: Record<AgenticPattern, Record<string, number>> = {
  "Reflection": { reasoning: 3, validation: 2, generation: 2, extraction: 1 },
  "Tool Use": { retrieval: 3, extraction: 2, summarization: 2, reasoning: 1 },
  "Planning": { reasoning: 3, orchestration: 2, monitoring: 2, generation: 1 },
  "ReAct Loop": { reasoning: 3, validation: 2, extraction: 2, monitoring: 1 },
  "Prompt Chaining": { generation: 3, extraction: 2, summarization: 2, routing: 1 },
  ...
  "Agent Handoff": { routing: 3, orchestration: 2, classification: 2, monitoring: 1 },
  "Parallelization": { orchestration: 3, generation: 2, extraction: 2, monitoring: 1 },
  "Generator-Critic": { generation: 3, validation: 2, summarization: 2, translation: 1 },
  "Group Chat": { reasoning: 3, generation: 2, orchestration: 2, validation: 1 },
};
...
const PATTERN_FUNCTION_SCORES: Record<AgenticPattern, Record<string, number>> = {
  "Reflection": { "R&D": 3, Marketing: 2, "Customer Service": 2, HR: 1 },
  "Tool Use": { "R&D": 3, Legal: 2, "Customer Service": 2, Sales: 1 },
  "Planning": { Operations: 3, IT: 2, "R&D": 2, Finance: 1 },
  "ReAct Loop": { IT: 3, "R&D": 2, Operations: 2, "Customer Service": 1 },
  "Prompt Chaining": { Marketing: 3, Sales: 2, HR: 2, Operations: 1 },
  ...
  "Agent Handoff": { "Customer Service": 3, Sales: 2, Operations: 2, IT: 1 },
  "Parallelization": { Operations: 3, IT: 3, Finance: 2, "R&D": 2 },
  "Generator-Critic": { Marketing: 3, Legal: 3, "R&D": 2, Sales: 1 },
  "Group Chat": { "R&D": 3, Executive: 2, Operations: 2, IT: 1 },
};
...
export function mapAgenticPatterns(useCase: UseCase): AgenticPatternMapping {
  ...
  const sortedPatterns = Array.from(scores.entries())
    .filter(([pattern]) => pattern !== "Constitutional Guardrail")
    .sort((a, b) => b[1] - a[1]);
  ...
  return {
    ...
    secondaryRationale,
    hitlPattern: "Constitutional Guardrail",
    hitlRationale,
...
function generatePatternRationale(
  ...
  const rationales: Record<AgenticPattern, string> = {
    "Reflection": `Reflection pattern enables self-critique of ${primitivesText} outputs for ${businessFunction}. The agent reviews its own work, identifies issues, and iteratively improves quality without external feedback.`,
    "Tool Use": `Tool Use pattern enhances ${primitivesText} with external tool integration for ${businessFunction}. The agent searches databases, APIs, and documents to ground responses in authoritative information.`,
    "Planning": `Planning pattern decomposes complex ${primitivesText} tasks for ${businessFunction}. The agent creates step-by-step plans, adapts to results, and coordinates execution of multi-stage workflows.`,
    ...
    "Prompt Chaining": `Prompt Chaining pattern sequences ${primitivesText} operations for ${businessFunction}. Each step's output feeds the next, enabling complex multi-step transformations with quality gates between stages.`,
    ...
    "Agent Handoff": `Agent Handoff pattern delegates specialized ${primitivesText} tasks for ${businessFunction}. When the primary agent encounters domain-specific needs, it transfers to a specialist agent with appropriate context.`,
    "Parallelization": `Parallelization pattern accelerates ${primitivesText} for ${businessFunction}. Multiple agents process subtasks concurrently, with results aggregated for faster throughput and comprehensive coverage.`,
    "Generator-Critic": `Generator-Critic pattern ensures high-quality ${primitivesText} outputs for ${businessFunction}. One agent generates content while another evaluates and refines it, achieving publication-quality results through structured iteration.`,
    "Group Chat": `Group Chat pattern enables collaborative ${primitivesText} for ${businessFunction}. Multiple specialized agents debate and build on each other's contributions to reach well-rounded solutions.`,
  };
