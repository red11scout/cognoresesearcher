...
import * as scenarioStore from "./scenario-store";
import * as customDataStore from "./custom-data-store";
...
Return ONLY valid JSON with this structure:
{
  ...
  // List all scenarios for a report
  app.get("/api/reports/:reportId/scenarios", async (req, res) => {
    try {
      const { reportId } = req.params;
      const scenarios = await scenarioStore.getScenariosByReport(reportId);
      return res.json(scenarios);
    } catch (error: any) {
      console.error("Error fetching scenarios:", error);
      return res.status(500).json({ error: error.message || "Failed to fetch scenarios" });
    }
  });

  // Create a new scenario for a report
  app.post("/api/reports/:reportId/scenarios", async (req, res) => {
    try {
      const { reportId } = req.params;
      const { name, description, assumptions, results } = req.body;
  ...
  // Compare multiple scenarios (query param: ids=id1,id2,id3)
  app.get("/api/reports/:reportId/scenarios/compare", async (req, res) => {
    try {
      ...
      const result = await scenarioStore.compareScenarios(ids);
      return res.json(result);
    } catch (error: any) {
      console.error("Error comparing scenarios:", error);
      return res.status(500).json({ error: error.message || "Failed to compare scenarios" });
    }
  });
  ...
  app.get("/api/scenarios/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const scenario = await scenarioStore.getScenarioById(id);
      if (!scenario) {
      ...
      return res.status(500).json({ error: error.message || "Failed to fetch scenario" });
    }
  });
  ...
  app.put("/api/scenarios/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const { name, description, assumptionsJson, resultsJson } = req.body;

      const updateData: { name?: string; description?: string; assumptionsJson?: any; resultsJson?: any } = {};
      ...
      const updated = await scenarioStore.updateScenario(id, updateData);
      if (!updated) {
        return res.status(404).json({ error: "Scenario not found" });
      }
      return res.json(updated);
  ...
  // ===== CUSTOM DATA ENDPOINTS (User-Added Themes, Friction Points, Use Cases) =====

  // List all custom data for a report
  app.get("/api/reports/:reportId/custom-data", async (req, res) => {
    try {
      const { reportId } = req.params;
      const items = await customDataStore.getCustomDataByReport(reportId);
      return res.json(items);
    } catch (error: any) {
      console.error("Error fetching custom data:", error);
      return res.status(500).json({ error: error.message || "Failed to fetch custom data" });
    }
  });

  // List custom data by type for a report
  app.get("/api/reports/:reportId/custom-data/:type", async (req, res) => {
    try {
      const { reportId, type } = req.params;
      const validTypes = ["theme", "friction_point", "use_case"];
      if (!validTypes.includes(type)) {
        return res.status(400).json({ error: `Invalid data type. Must be one of: ${validTypes.join(", ")}` });
      }
      const items = await customDataStore.getCustomDataByReport(reportId, type);
      return res.json(items);
    } catch (error: any) {
      console.error("Error fetching custom data by type:", error);
      return res.status(500).json({ error: error.message || "Failed to fetch custom data" });
    }
  });

  // Create a custom data item
  app.post("/api/reports/:reportId/custom-data", async (req, res) => {
    try {
      const { reportId } = req.params;
      const { dataType, dataJson } = req.body;

      if (!dataType) {
        return res.status(400).json({ error: "dataType is required (theme, friction_point, or use_case)" });
      }
      const validTypes = ["theme", "friction_point", "use_case"];
      ...
      if (!dataJson || typeof dataJson !== "object") {
        return res.status(400).json({ error: "dataJson is required and must be an object" });
      }

      const record = await customDataStore.createCustomData(reportId, dataType, dataJson);
      return res.status(201).json(record);
    } catch (error: any) {
      console.error("Error creating custom data:", error);
      return res.status(500).json({ error: error.message || "Failed to create custom data" });
    }
  });

  // Update a custom data item
  app.put("/api/custom-data/:id", async (req, res) => {
    try {
      const { id } = req.params;
      ...
      const updated = await customDataStore.updateCustomData(id, dataJson);
      if (!updated) {
        return res.status(404).json({ error: "Custom data item not found" });
      }
      return res.json(updated);
    } catch (error: any) {
      console.error("Error updating custom data:", error);
      return res.status(500).json({ error: error.message || "Failed to update custom data" });
    }
  });

  // Delete a custom data item
  app.delete("/api/custom-data/:id", async (req, res) => {
    try {
      const { id } = req.params;
      await customDataStore.deleteCustomData(id);
      return res.json({ success: true, message: "Custom data item deleted" });
    } catch (error: any) {
      console.error("Error deleting custom data:", error);
      return res.status(500).json({ error: error.message || "Failed to delete custom data" });
    }
  });

  // ===== VERIFY CALCULATION ENDPOINT =====

  // Verify a calculation by evaluating a formula against expected output
  app.post("/api/verify-calculation", async (req, res) => {
    try {
      const { inputs, formula, expectedOutput } = req.body;

      if (!inputs || !formula || expectedOutput === undefined) {
        return res.status(400).json({
          error: "Required fields: inputs (object), formula (string), expectedOutput (number)",
        });
      }

      // Build a simple evaluator that resolves variable references from inputs
      // Formula is expected as a string like "hoursSaved * loadedHourlyRate * adoptionRate"
      let actualOutput: number;
      try {
        // Create a safe evaluation context from the input variables
        const keys = Object.keys(inputs);
        const values = keys.map(k => inputs[k]);
        // eslint-disable-next-line no-new-func
        const fn = new Function(...keys, `"use strict"; return (${formula});`);
        actualOutput = fn(...values);
      } catch (evalError: any) {
        return res.status(400).json({
      ...
      const diff = actualOutput - expectedOutput;
      const verified = Math.abs(diff) < 0.01; // tolerance for floating point
      ...
      return res.status(500).json({ error: error.message || "Failed to verify calculation" });
    }
  });
